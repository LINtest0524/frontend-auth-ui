...
export default function PromotionDetailPage() {
  ...
  const fetchPromotion = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE}/portal/promotions/${id}?company=${companyCode}`)
  ...
  const incrementViewCount = async () => {
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_BASE}/portal/promotions/${id}/view?company=${companyCode}`, {
        method: 'POST'
      })
    } catch (error) {
      // 更新瀏覽次數失敗，靜默處理
