...
export default function FlexCumulativePage() {
    ...
    <div className="checkin-container fade-in">
      <div className="checkin-header">
        <div className="checkin-header-content">
          <h1>
            累積簽到活動
            <span className="activity-type-badge flex-cumulative">彈性累積</span>
          </h1>
          <p>管理期間累積簽到活動，用戶在活動期間內累積簽到天數即可，無需連續簽到</p>
        </div>
        <Link href="/checkin/flex-cumulative/new" className="checkin-add-btn">
          新增累積簽到活動
        </Link>
      </div>
                ...
                activities.map((activity, index) => (
                  <tr key={activity.id} className="table-row-enter" style={{ animationDelay: `${index * 0.1}s` }}>
                    <td>
                      ...
                      <div className="activity-subtitle">累積簽到活動 • 期間內累積簽到天數</div>
                    </td>
                    <td>
                      <div className="date-range">
                        <div className="date-item">
                          <span className="date-icon">📅</span>
                          開始：{activity.startDate}
                        </div>
                        <div className="date-item">
                          <span className="date-icon">🏁</span>
                          結束：{activity.endDate}
