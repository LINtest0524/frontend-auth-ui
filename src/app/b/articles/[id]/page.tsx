...
async function getArticleData(articleId: string): Promise<ArticleResponse | null> {
  try {
    ...
    const response = await fetch(
      `${apiBase}/portal/articles/${articleId}?company=b`,
      { 
...
export async function generateMetadata({ 
  ...
  return {
    title: `${article.title} | 代理商公司B`,
    description: article.summary || article.content.substring(0, 160),
    keywords: `文章,${article.category.name},代理商公司B`,
    authors: [{ name: '代理商公司B' }],
    ...
    alternates: {
      canonical: `${baseUrl}/b/articles/${article.id}`,
    },
  }
}
...
export default async function ArticleDetailPage({ 
  ...
  return (
    <>
      <PortalHeaderBar />
      <ArticleDetailClient articleData={articleData} companyCode="b" />
    </>
  )
}